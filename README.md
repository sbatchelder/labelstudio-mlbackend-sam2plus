...
1. **Clone the repository.** The bundled example images and probe artifacts are
   stored with [Git LFS](https://git-lfs.com/), so install it first; the clone
   then fetches those files automatically.
   ...
   If you cloned before installing LFS — the `.jpg`/`.png` files show up as small
   text pointer stubs instead of real images — fetch them with `git lfs pull`.

   These LFS files are optional — the backend doesn't need them. They hold the
   example input imagery plus the subpatching/postprocessing intermediates and
   outputs under `examples/`, all generated by the bundled `sam2plus-probe`
